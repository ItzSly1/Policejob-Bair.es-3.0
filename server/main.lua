...
AddEventHandler('esx_policejob:handcuff', function(target)
	...
	if xPlayer.job.name == 'police' then
		...
		TriggerClientEvent('bm_mafias:arrested', target, source)
...
RegisterServerEvent('startFleccasRobbery')	
AddEventHandler('startFleccasRobbery', function()
    local players = GetPlayers()

    for _, playerId in ipairs(players) do
        TriggerClientEvent('startFleccasRobberyNotification', playerId)
    end
end)

RegisterNetEvent('startFleccasRobberyNotification')
AddEventHandler('startFleccasRobberyNotification', function()
    TriggerEvent('mythic_notify:client:SendAlert', { 
        type = 'inform', 
        text = 'SE NEGOCIACIONES EN FLECCA. COMIENZAN LOS 10SG DE GRACIA' 
    })
    ...
    TriggerEvent('mythic_notify:client:SendAlert', { 
        type = 'inform', 
        text = 'COMIENZA EL TIROTEO EN FLECCA' 
    })

    -- Ejecutar el comando /pfa
    ExecuteCommand('pfa')
end)

RegisterNetEvent('esx_policejob:requestSearch', function(id)
	local player = ESX.GetPlayerFromId(source)

	if (player.getJob().name == 'police') then
		TriggerClientEvent("inventory:openPlayerInventory", source, id)
	end
end)
